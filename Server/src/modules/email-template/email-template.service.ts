...
export class EmailTemplateService {
  ...
  async getScheduledEmail() {
    const results = await this._emailTemplateRepository.find();
    // const result = [];
    const entityManager = getManager();
    const res = await entityManager.query(
      `SELECT * FROM "tblGroups" tg inner join "tblUsers" tu on tg.id = tu."groupId" where "groupName" = 'Alchemy'`,
    );
    return res;
    // results.map((c) => {
    //   const groups = JSON.parse(c.groupName);
    //   const attachments = JSON.parse(c.attachment);

    //   const res = groups.map(async (g) => {
    //     const entityManager = getManager();
    ...
    const array = results.filter((r) => r.isSent === 'N');
    return results;
  }
  ...
  async sendEmail(emailDetails) {
    ...
    /*  const mail = {
      to: 'ragavinrap@gmail.com',
      subject: 'Hello from sendgrid',
      from: 'inquiries@walletwizard.com',
      text: 'Hello',
      html: '<h1>Hello</h1>',
      // attachments: data,
    };
    return await this.client.send(mail); */
  }
}
